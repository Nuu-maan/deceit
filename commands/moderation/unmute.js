...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!memberToUnmute) {
      infoEmbed.setFooter({ text: 'Mention a user to unmute.', iconURL: message.guild.iconURL({ dynamic: true }) });
      return message.channel.send({ embeds: [infoEmbed] });
    }

    if (!message.member.permissions.has(PermissionsBitField.Flags.MuteMembers)) {
      errorEmbed.setDescription('You need the `MUTE_MEMBERS` permission to use this command.');
      return message.channel.send({ embeds: [errorEmbed] });
    }

    if (!message.guild.members.me.permissions.has(PermissionsBitField.Flags.MuteMembers)) {
      errorEmbed.setDescription('I don’t have the `MUTE_MEMBERS` permission to unmute this user.');
      return message.channel.send({ embeds: [errorEmbed] });
    }

    if (message.guild.members.me.roles.highest.comparePositionTo(memberToUnmute.roles.highest) <= 0) {
      errorEmbed.setDescription('I cannot unmute this member due to role hierarchy.');
      return message.channel.send({ embeds: [errorEmbed] });
    }
    ...
    try {
      ...
      successEmbed
        ...
        .setFooter({ text: `Unmute executed successfully by ${message.author.tag}`, iconURL: message.author.displayAvatarURL({ dynamic: true }) })
        .setTimestamp();
      ...
      console.error(error);
      message.reply({ content: 'There was an error trying to unmute the user.' });
    }
  },
};
