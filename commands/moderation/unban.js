...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (
      ...
      errorEmbed.setDescription(
        'I don’t have the `BAN_MEMBERS` permission to unban this user.',
      );
      return message.channel.send({ embeds: [errorEmbed] });
    }
    ...
    try {
      ...
      successEmbed
        ...
        .setDescription(
          `\`${bannedUser.tag}\` has been unbanned from the server.`,
        )
        .setFields(
      ...
      errorEmbed.setDescription(
        'There was an error trying to unban the user. They may not be banned or the ID is incorrect.',
      );
      return message.channel.send({ embeds: [errorEmbed] });
    }
  },
